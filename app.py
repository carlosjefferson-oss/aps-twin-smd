...
def usuario_enxerga_tudo():
    return perfil_atual() in ["adm", "aluno", "secretario"]
...
def aplicar_restricao_de_perfil(df):
    ...
    if perfil == "populacao":
        df_municipal = df[df["ubs"].astype(str).str.lower().isin(["aps municipal", "municipal"])]
        if not df_municipal.empty:
            return df_municipal.copy()
...
st.caption(
    f"Usuário: {st.session_state.get('nome_usuario', '')} | Perfil: {st.session_state.get('perfil_usuario', '').upper()} | Escopo: {st.session_state.get('ubs_usuario', 'Todas')}"
)
...
nav_botoes = [
    ...
    ("🏥 UBS e Território", "🏥 UBS e Território"),
    ("📈 Séries Temporais", "📈 Séries Temporais"),
    ("🕸️ Sistematicidade", "🕸️ Sistematicidade"),
    ("📄 Relatórios", "📄 Relatórios"),
    ("ℹ️ Sobre o SMD", "ℹ️ Sobre o SMD"),
]

if usuario_pode_api():
    nav_botoes.insert(1, ("🧪 Qualidade dos Dados/API", "🧪 Qualidade dos Dados"))
    nav_botoes.insert(5, ("🤖 Machine Learning", "🤖 Machine Learning"))
    nav_botoes.insert(6, ("🧠 Digital Twin", "🧠 Digital Twin Ciberfísico"))
elif not usuario_e_publico():
    nav_botoes.insert(5, ("🤖 Machine Learning", "🤖 Machine Learning"))
    nav_botoes.insert(6, ("🧠 Digital Twin", "🧠 Digital Twin Ciberfísico"))

for linha, inicio in enumerate(range(0, len(nav_botoes), 5)):
    nav_cols = st.columns(5)
    for j, (rotulo, destino) in enumerate(nav_botoes[inicio:inicio + 5]):
        with nav_cols[j]:
            label = f"✅ {rotulo}" if st.session_state.get("aba_ativa") == destino else rotulo
            if st.button(label, key=f"nav_principal_{linha}_{j}", use_container_width=True):
...
if usuario_pode_sidebar():
    ...
    if usuario_e_gerente():
        ...
        ubs_sel = ["APS Municipal"] if "APS Municipal" in ubs_disponiveis else ubs_disponiveis
    else:
...
# O mapa usa o recorte filtrado. Se o recorte ficar sem UBS, usa a base completa para não desaparecer.
dados_cards_base = aplicar_restricao_de_perfil(dados_f.copy())
if dados_cards_base.empty or "ubs" not in dados_cards_base.columns:
    dados_cards_base = aplicar_restricao_de_perfil(dados.copy())
...
ubs_validas_cards = [
    ...
    if str(u).strip() != ""
    and str(u).lower() not in ["aps municipal", "todas", "nan", "none"]
    and "2024" not in str(u)
    and "/" not in str(u)
]

# Fallback para a base completa se filtros laterais removerem todas as UBS.
if len(ubs_validas_cards) == 0:
    dados_cards_base = aplicar_restricao_de_perfil(dados.copy())
    dados_cards_base["ubs"] = dados_cards_base["ubs"].apply(normalizar_nome_ubs)
    ubs_validas_cards = [
        u for u in sorted(set(dados_cards_base["ubs"].dropna().astype(str).tolist()))
...
if st.session_state.get("perfil_usuario") == "ubs":
    ubs_do_usuario = normalizar_nome_ubs(st.session_state.get("ubs_usuario"))
    ubs_validas_cards = [
        u for u in ubs_validas_cards
        if normalizar_nome_ubs(u) == ubs_do_usuario
    ]

if len(ubs_validas_cards) == 0:
    st.info("Nenhuma UBS disponível para o recorte atual. Ajuste os filtros na lateral ou verifique a base de dados.")
else:
    resumo_cards_ubs = (
        dados_cards_base[dados_cards_base["ubs"].isin(ubs_validas_cards)]
        .groupby("ubs")
        .agg(
    ...
    for pos, row in enumerate(resumo_cards_ubs.itertuples(index=False)):
        ubs_nome = row.ubs

        with colunas_cards[pos % 4]:
            exibir_card_ubs(
                ubs_nome,
                row.score_ubs,
                int(row.crit_ubs),
            ...
            if st.button("Abrir painel", key=f"btn_mapa_ubs_{ubs_nome}_{pos}", use_container_width=True):
                st.session_state["ubs_selecionada_card"] = ubs_nome
                st.session_state["aba_ativa"] = "🏥 UBS e Território"
                st.rerun()
...
if st.session_state.get("ubs_selecionada_card", "Todas") != "Todas":
    ...
    if st.button("Voltar para visão geral municipal", use_container_width=False, key="voltar_visao_geral_municipal"):
        st.session_state["ubs_selecionada_card"] = "Todas"
        st.rerun()
