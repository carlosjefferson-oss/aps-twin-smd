...
def tela_login():
    """
    Tela de login mais limpa, moderna e proporcional.
    """
    st.markdown(
        """
        <style>
        ...
        with st.container(border=True):
            ...
            with img_ctr:
                if logo_pequena.exists():
                    st.image(str(logo_pequena), width=150)
...
def normalizar_nome_ubs(nome):
    """Padroniza nomes de UBS para evitar duplicidades como PAU FERRO e Pau Ferro."""
    if pd.isna(nome):
        return nome
...
Relatório gerado automaticamente pelo APS-Twin SMD v1.4.
"""
    return relatorio
...
for i, row in resumo_cards_ubs.iterrows():
    ...
    with colunas_cards[i % 3]:
        ...
        if st.button(f"Abrir painel da UBS", key=f"btn_{ubs_nome}", use_container_width=True):
...
st.divider()
st.caption(
    "APS-Twin SMD v1.4 | Protótipo web em Python | API em tempo real | Qualidade dos Dados | Relatórios Executivos | Digital Twin Ciberfísico-Gerencial da APS | "
