...
# =========================================================
# APS-Twin SMD v1.8.8.7.6.5.4.3.2
...
Relatório gerado automaticamente pelo APS-Twin SMD v1.8.8.7.7.6.6.5.5.4.4.3.3.2.2.1.
"""
    return relatorio
...
for i, (rotulo, destino) in enumerate(nav_botoes[:5]):
    with nav_cols_1[i]:
        label = f"✅ {rotulo}" if st.session_state.get("aba_ativa") == destino else rotulo
        if st.button(label, key=f"nav_principal_l1_{i}", use_container_width=True):
            st.session_state["aba_ativa"] = destino
            st.rerun()

nav_cols_2 = st.columns(5)
for j, (rotulo, destino) in enumerate(nav_botoes[5:]):
    with nav_cols_2[j]:
        label = f"✅ {rotulo}" if st.session_state.get("aba_ativa") == destino else rotulo
        if st.button(label, key=f"nav_principal_l2_{j}", use_container_width=True):
            st.session_state["aba_ativa"] = destino
            st.rerun()
...
st.divider()
st.caption(
    "APS-Twin SMD v1.8.8.7.6.5.4.3.2 | Protótipo web em Python | API em tempo real | Qualidade dos Dados | Relatórios Executivos | Digital Twin Ciberfísico-Gerencial da APS | "
