...
def render_header_principal():
    """Cabeçalho institucional com bandeiras de Pernambuco e Salgueiro."""
    ...
    if flag_pe_b64:
        flag_pe_html = f"""
        <div class="hero-flag-card">
            <img src="data:image/png;base64,{flag_pe_b64}" class="hero-flag">
            <div class="hero-flag-label">Pernambuco</div>
        </div>
        """

    if flag_salgueiro_b64:
        flag_salgueiro_html = f"""
        <div class="hero-flag-card">
            <img src="data:image/png;base64,{flag_salgueiro_b64}" class="hero-flag">
            <div class="hero-flag-label">Salgueiro-PE</div>
        </div>
        """
    ...
    st.markdown(
        ...
        <div class="hero-wrapper">
            <div class="hero-grid">
                <div class="hero-text">
                    <div class="hero-title">APS-Twin SMD</div>
                    <div class="hero-subtitle">
                        Digital Twin Gerencial da Atenção Primária à Saúde | Salgueiro-PE
                    </div>
                    <div class="hero-line">
                        18 indicadores • Monitoramento longitudinal • Machine Learning •
                        Sistematicidade • Sistema ciberfísico • API em tempo real • Apoio à decisão
                    </div>
...
def normalizar_nome_ubs(nome):
    """Padroniza nomes de UBS para evitar duplicidades entre bases e API."""
    if pd.isna(nome):
        return nome
    ...
    canon = unicodedata.normalize("NFKD", txt).encode("ascii", "ignore").decode("utf-8").lower()
...
def detectar_arquivos():
    ...
    for arquivo in arquivos:
        try:
            ...
            if "ubs" in colunas:
                if arquivo_ubs is None or arquivo.name.lower().startswith("indicadores_ubs"):
                    arquivo_ubs = arquivo.name
            else:
                if arquivo_municipal is None or arquivo.name.lower().startswith("indicadores_aps"):
...
Relatório gerado automaticamente pelo APS-Twin SMD v1.7.
"""
    return relatorio
...
if dados is None:
    ...
    st.write("Verifique se a pasta data contém:")
    st.code("""
data/dictionary_indicators.csv
data/indicadores_aps.xlsx
data/indicadores_ubs.xlsx
""")
    st.stop()
...
st.divider()
st.caption(
    "APS-Twin SMD v1.7 | Protótipo web em Python | API em tempo real | Qualidade dos Dados | Relatórios Executivos | Digital Twin Ciberfísico-Gerencial da APS | "
