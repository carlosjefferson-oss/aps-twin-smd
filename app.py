...
def tela_login():
    """
    Tela de login limpa, sem DIVs abertas atravessando componentes Streamlit.
    Isso evita o aparecimento daquela faixa/card vazio no topo da página.
    """
    st.markdown(
        """
        <style>
    ...
    # Centraliza a tela sem usar HTML container aberto.
    esquerda, centro, direita = st.columns([1.35, 1.0, 1.35])

    with centro:
        # Card Streamlit nativo: evita quebra visual e funciona melhor no navegador.
        with st.container(border=True):
            logo_pequena = ASSETS_DIR / "aps_twin_logo_square_600x600.png"

            img_esq, img_centro, img_dir = st.columns([1.25, 1.0, 1.25])
            with img_centro:
                if logo_pequena.exists():
                    st.image(str(logo_pequena), use_container_width=True)
            ...
            st.markdown(
                ...
                <div class="login-subtitle-clean">
                    ...
                    Indicadores, UBS, previsões, sistematicidade e simulações.
                </div>
                """,
                unsafe_allow_html=True
            )
            ...
            if entrar:
                ...
                if usuario_limpo in USUARIOS and senha == USUARIOS[usuario_limpo]["senha"]:
                    ...
                    st.session_state["ubs_usuario"] = USUARIOS[usuario_limpo]["ubs"]

                    if USUARIOS[usuario_limpo]["perfil"] == "ubs":
                        st.session_state["ubs_selecionada_card"] = USUARIOS[usuario_limpo]["ubs"]
                    else:
...
def exibir_card_ubs(nome_ubs, score, criticos, atencao, adequados):
    ...
    st.markdown(
        ...
        <div class="{classe}">
            ...
            <div class="ubs-small">Críticos: {criticos} | Atenção: {atencao} | Adequados: {adequados}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
...
Relatório gerado automaticamente pelo APS-Twin SMD v1.3.
"""
    return relatorio
...
st.markdown(
    ...
    <div class="app-header">
        <div class="app-title">APS-Twin SMD</div>
        <div class="app-subtitle">
            Digital Twin Gerencial da Atenção Primária à Saúde | Salgueiro-PE
        </div>
        <div class="app-tags">
            18 indicadores • Monitoramento longitudinal • Machine Learning • Sistematicidade • Sistema ciberfísico • API em tempo real • Apoio à decisão
        </div>
    </div>
    """,
    unsafe_allow_html=True
...
ubs_lista = sorted(dados["ubs"].dropna().unique())
...
st.markdown('<div class="section-title">Painel de acesso rápido por UBS</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="section-caption">Clique em uma unidade para filtrar automaticamente todas as análises, previsões e simulações.</div>',
    unsafe_allow_html=True
)

ubs_validas_cards = [
    u for u in sorted(dados["ubs"].dropna().unique())
...
if st.session_state.get("perfil_usuario") == "ubs":
    ubs_validas_cards = [
        ...
        if u == st.session_state.get("ubs_usuario")
    ]

colunas_cards = st.columns(3)

for i, ubs_nome in enumerate(ubs_validas_cards):
    dados_ubs_card = dados[dados["ubs"] == ubs_nome].copy()

    score_ubs = dados_ubs_card["score"].mean()
    crit_ubs = (dados_ubs_card["status"] == "Crítico").sum()
    aten_ubs = (dados_ubs_card["status"] == "Atenção").sum()
    adeq_ubs = (dados_ubs_card["status"] == "Adequado").sum()

    with colunas_cards[i % 3]:
        exibir_card_ubs(ubs_nome, score_ubs, crit_ubs, aten_ubs, adeq_ubs)

        if st.button(f"Abrir painel de {ubs_nome}", key=f"btn_{ubs_nome}", use_container_width=True):
            st.session_state["ubs_selecionada_card"] = ubs_nome
            st.rerun()

if st.session_state["ubs_selecionada_card"] != "Todas":
    st.success(f"Painel filtrado para: {st.session_state['ubs_selecionada_card']}")
...
st.divider()
st.caption(
    "APS-Twin SMD v1.3 | Protótipo web em Python | API em tempo real | Qualidade dos Dados | Relatórios Executivos | Digital Twin Ciberfísico-Gerencial da APS | "
