...
def render_flag_card(path, label):
    """Renderiza uma bandeira em card nativo do Streamlit."""
    with st.container(border=True):
        if path.exists():
            st.image(str(path), use_container_width=True)
        else:
            st.markdown(
                "<div style='height:78px;display:flex;align-items:center;justify-content:center;color:#667085;'>Bandeira não encontrada</div>",
                unsafe_allow_html=True
            )
        st.markdown(
            f"<div style='text-align:center;color:#0B2545;font-size:12px;font-weight:900;margin-top:4px;'>{label}</div>",
            unsafe_allow_html=True
        )
...
def render_header_principal():
    """
    Cabeçalho institucional com bandeiras, usando layout nativo do Streamlit.
    Esta versão evita HTML quebrado aparecendo na tela.
    """
    st.markdown(
        """
        <style>
            ...
            .hero-clean-wrapper div[data-testid="stVerticalBlockBorderWrapper"] {
    ...
    with st.container():
        st.markdown('<div class="hero-clean-wrapper">', unsafe_allow_html=True)

        col_texto, col_bandeiras = st.columns([3.1, 1.15], gap="large")

        with col_texto:
            st.markdown('<div class="hero-clean-title">APS-Twin SMD</div>', unsafe_allow_html=True)
            st.markdown(
                '<div class="hero-clean-subtitle">Digital Twin Gerencial da Atenção Primária à Saúde | Salgueiro-PE</div>',
                unsafe_allow_html=True
            )
            st.markdown(
                '<div class="hero-clean-line">18 indicadores • Monitoramento longitudinal • Machine Learning • Sistematicidade • Sistema ciberfísico • API em tempo real • Apoio à decisão</div>',
                unsafe_allow_html=True
            )
            st.markdown(
                """
                <span class="hero-badge">🏥 UBS</span>
                <span class="hero-badge">📊 SMD</span>
                <span class="hero-badge">🤖 ML</span>
                <span class="hero-badge">🧠 Digital Twin</span>
                <span class="hero-badge">🔌 API</span>
                """,
                unsafe_allow_html=True
            )

        with col_bandeiras:
            st.markdown('<div class="hero-flags-title">Identidade territorial</div>', unsafe_allow_html=True)
            flag_col1, flag_col2 = st.columns(2, gap="small")
            with flag_col1:
                render_flag_card(FLAG_SALGUEIRO_PATH, "Salgueiro-PE")
            with flag_col2:
                render_flag_card(FLAG_PE_PATH, "Pernambuco")
...
def tela_login():
    """
    Tela de login institucional, limpa e proporcional.
    """
    st.markdown(
        """
        <style>
    ...
    esq, centro, dir_ = st.columns([1.2, 1.0, 1.2])

    with centro:
        with st.container(border=True):
            st.markdown('<div class="login-institutional-badge">APS-Twin SMD • v1.6 institucional</div>', unsafe_allow_html=True)

            logo_pequena = ASSETS_DIR / "aps_twin_logo_square_600x600.png"
            img_esq, img_ctr, img_dir = st.columns([1.1, 1.2, 1.1])
            with img_ctr:
                if logo_pequena.exists():
                    st.image(str(logo_pequena), width=155)
                elif LOGO_PATH is not None:
                    st.image(str(LOGO_PATH), width=155)
                else:
                    st.markdown("<div style='text-align:center; font-size:76px;'>🏥</div>", unsafe_allow_html=True)
            ...
            if entrar:
                ...
                if usuario_limpo in USUARIOS and senha == USUARIOS[usuario_limpo]["senha"]:
                    ...
                    if USUARIOS[usuario_limpo]["perfil"] == "ubs":
                        st.session_state["ubs_selecionada_card"] = normalizar_nome_ubs(USUARIOS[usuario_limpo]["ubs"])
                    else:
            ...
            st.markdown(
                ...
                <div class="login-helper-clean">
                    ...
                    Usuário: <b>admin</b> &nbsp;|&nbsp; Senha: <b>aps123</b><br>
                    Perfis: administrador, gestor municipal e acesso restrito por UBS.
                </div>
                """,
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            '<div class="login-footer-clean">APS-Twin SMD • Protótipo tecnológico derivado da tese</div>',
            unsafe_allow_html=True
        )
...
Relatório gerado automaticamente pelo APS-Twin SMD v1.7.3.3.3.2.2.1.
"""
    return relatorio
...
meses_lista = sorted(dados["mes"].astype(str).dropna().unique())
...
st.markdown(
    '<div class="section-caption">Cards consolidados por UBS com score, criticidade e acesso direto às análises específicas.</div>',
    unsafe_allow_html=True
)

ubs_validas_cards = [
    u for u in sorted(set(dados["ubs"].dropna().astype(str).tolist()))
    if str(u).lower() not in ["aps municipal"]
...
if st.session_state.get("perfil_usuario") == "ubs":
    ubs_validas_cards = [
        u for u in ubs_validas_cards
        if u == normalizar_nome_ubs(st.session_state.get("ubs_usuario"))
    ]

resumo_cards_ubs = (
    dados[dados["ubs"].isin(ubs_validas_cards)]
    .groupby("ubs")
    .agg(
...
colunas_cards = st.columns(3)

for i, row in resumo_cards_ubs.iterrows():
    ubs_nome = row["ubs"]

    with colunas_cards[i % 3]:
        exibir_card_ubs(
            ubs_nome,
            row["score_ubs"],
            int(row["crit_ubs"]),
        ...
        if st.button("Abrir painel desta UBS", key=f"btn_{ubs_nome}", use_container_width=True):
            st.session_state["ubs_selecionada_card"] = ubs_nome
            st.rerun()

if st.session_state["ubs_selecionada_card"] != "Todas":
    st.success(f"Painel filtrado para a UBS: {st.session_state['ubs_selecionada_card']}")
    if st.button("Voltar para visão geral municipal", use_container_width=False):
        st.session_state["ubs_selecionada_card"] = "Todas"
        st.rerun()
...
st.markdown(
    '<div class="section-caption">Navegação inteligente do sistema: utilize as abas abaixo para explorar os módulos analíticos, operacionais e preditivos do APS-Twin SMD.</div>',
    unsafe_allow_html=True
)
...
st.divider()
st.caption(
    "APS-Twin SMD v1.7.3.3.2 | Protótipo web em Python | API em tempo real | Qualidade dos Dados | Relatórios Executivos | Digital Twin Ciberfísico-Gerencial da APS | "
