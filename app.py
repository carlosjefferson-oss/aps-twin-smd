...
# =========================================================
# APS-Twin SMD v1.8.0.7.6.5.4.3.2
...
def render_header_principal():
    """
    Cabeçalho institucional em HTML único, com atalhos dentro da faixa azul
    e logos institucionais abaixo das bandeiras.
    """
    ...
    salgueiro_html = (
        f'<div class="flag-card"><img src="data:image/png;base64,{flag_salgueiro_b64}" class="flag-img" />'
        f'<div class="flag-label">Salgueiro-PE</div></div>'
        if flag_salgueiro_b64
        else '<div class="flag-card flag-missing"><div class="flag-placeholder">Salgueiro-PE</div></div>'
    )

    pe_html = (
        f'<div class="flag-card"><img src="data:image/png;base64,{flag_pe_b64}" class="flag-img" />'
        f'<div class="flag-label">Pernambuco</div></div>'
        if flag_pe_b64
        else '<div class="flag-card flag-missing"><div class="flag-placeholder">Pernambuco</div></div>'
    )

    univasf_html = (
        f'<div class="inst-card"><img src="data:image/png;base64,{univasf_b64}" class="inst-img" />'
        f'<div class="inst-label">UNIVASF</div></div>'
        if univasf_b64
        else '<div class="inst-card"><div class="inst-placeholder">UNIVASF</div><div class="inst-label">UNIVASF</div></div>'
    )

    ufba_html = (
        f'<div class="inst-card"><img src="data:image/png;base64,{ufba_b64}" class="inst-img" />'
        f'<div class="inst-label">UFBA</div></div>'
        if ufba_b64
        else '<div class="inst-card"><div class="inst-placeholder">UFBA</div><div class="inst-label">UFBA</div></div>'
    )
...
.hero-main {{
    width: 100%;
    box-sizing: border-box;
    background: radial-gradient(circle at 92% 10%, rgba(255,255,255,0.24), transparent 28%),
...
@media (max-width: 980px) {{
    ...
    .flags-title, .institutions-title {{
        text-align: left;
    }}
    .flags-grid, .institutions-grid {{
        max-width: 320px;
    }}
}}
</style>
<div class="hero-main">
  <div class="hero-grid-html">
    <div class="hero-text-block">
      ...
      <div class="hero-shortcuts-title">Atalhos rápidos</div>
      <div class="hero-shortcuts-grid">
        <a class="hero-shortcut-link" href="?modulo=ubs">🏥 UBS</a>
        <a class="hero-shortcut-link" href="?modulo=smd">📊 SMD</a>
        <a class="hero-shortcut-link" href="?modulo=ml">🤖 ML</a>
        <a class="hero-shortcut-link" href="?modulo=dt">🧠 Digital Twin</a>
        <a class="hero-shortcut-link" href="?modulo=api">🔌 API</a>
      </div>
    </div>
    <div class="flags-area">
      <div class="flags-title">Identidade territorial</div>
      <div class="flags-grid">{salgueiro_html}{pe_html}</div>
      <div class="institutions-title">Instituições parceiras</div>
      <div class="institutions-grid">{univasf_html}{ufba_html}</div>
    </div>
  </div>
</div>
"""
...
Relatório gerado automaticamente pelo APS-Twin SMD v1.8.0.7.7.6.6.5.5.4.4.3.3.2.2.1.
"""
    return relatorio
...
MAPA_QUERY_MODULOS = {
    ...
    "smd": "📊 Painel Executivo",
    "ml": "🤖 Machine Learning",
    "dt": "🧠 Digital Twin Ciberfísico",
    "api": "🧪 Qualidade dos Dados",
}
try:
    modulo_qp = st.query_params.get("modulo", "")
    if isinstance(modulo_qp, list):
        modulo_qp = modulo_qp[0] if modulo_qp else ""
    modulo_qp = str(modulo_qp).strip().lower()
    if modulo_qp in MAPA_QUERY_MODULOS:
        st.session_state["aba_ativa"] = MAPA_QUERY_MODULOS[modulo_qp]
except Exception:
    pass
...
for i, row in resumo_cards_ubs.iterrows():
    ...
    with colunas_cards[i % 4]:
        ...
        if st.button("Abrir painel", key=f"btn_{ubs_nome}", use_container_width=True):
...
st.markdown(
    '<div class="section-caption">Navegação inteligente do sistema: utilize os atalhos e o seletor abaixo para explorar os módulos analíticos, operacionais e preditivos do APS-Twin SMD.</div>',
    unsafe_allow_html=True
)
...
aba_escolhida = st.radio(
    "Módulo do sistema",
    MODULOS_APS,
    index=MODULOS_APS.index(st.session_state["aba_ativa"]),
    horizontal=True,
    label_visibility="collapsed",
    key="radio_modulo_sistema"
)
...
st.divider()
st.caption(
    "APS-Twin SMD v1.8.0.7.6.5.4.3.2 | Protótipo web em Python | API em tempo real | Qualidade dos Dados | Relatórios Executivos | Digital Twin Ciberfísico-Gerencial da APS | "
